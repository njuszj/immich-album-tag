...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "tag_album" ADD CONSTRAINT "PK_tag_album_albumsId_tagsId" PRIMARY KEY ("albumsId", "tagsId");`.execute(db);
  await sql`ALTER TABLE "tag_album" ADD CONSTRAINT "FK_tag_album_albumsId" FOREIGN KEY ("albumsId") REFERENCES "albums" ("id") ON UPDATE CASCADE ON DELETE CASCADE;`.execute(db);
  await sql`ALTER TABLE "tag_album" ADD CONSTRAINT "FK_tag_album_tagsId" FOREIGN KEY ("tagsId") REFERENCES "tags" ("id") ON UPDATE CASCADE ON DELETE CASCADE;`.execute(db);
