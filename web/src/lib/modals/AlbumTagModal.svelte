<script lang="ts">
  import { tagAlbum } from '$lib/utils/album-utils';
  ...
  let { onClose, albumId }: Props = $props();
  ...
  const handleSubmit = async () => {
    await tagAlbum({ albumId, tagIds: [...selectedIds], showNotification: false });
    onClose(true);
  };
...
<Modal size="small" title={$t('tag_album')} icon={mdiTag} {onClose}>
  <ModalBody>
    <form {onsubmit} autocomplete="off" id="tag-album-form">
  ...
  <ModalFooter>
    <HStack fullWidth>
      ...
      <Button type="submit" shape="round" fullWidth form="tag-album-form" {disabled}>{$t('tag_album')}</Button>
    </HStack>
  </ModalFooter>
</Modal>